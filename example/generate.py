    ...
    for j in np.arange(len(snap)):
        for i in np.arange(len(mass)):
            ...
            for m in np.arange(len(cut)):
                ...
                #print >> f, 'python', '../scripts/showstacks_'+cut[m]+'.py', prof2+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz', prof3+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz', str(mass[i]), snap[j], sim[k],type1
                #print >> f, 'python', '../scripts/showstacks_'+cut[m]+'.py', prof2+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz', prof3+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz',str(mass[i]), snap[j], sim[k],type2
                print >> f, 'python', '../scripts/showstacks_'+cut[m]+'.py', prof2+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz', prof3+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz',str(mass[i]), snap[j], sim[k],type3
                print >> f, 'python', '../scripts/showstacks_'+cut[m]+'.py', prof2+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz', prof3+'_scaled_'+sim[k]+'_'+str(snap[j])+'_mstar_total.npz',str(mass[i]), snap[j], sim[k],type4
...
print("time elapsed", time_elapsed/60., "minutes")
