...
print "running showstacks_sfr.py"
cut_sfr=1
...
#make histogram to see cut
percentile_16=np.percentile(sfr,16)
percentile_30=np.percentile(sfr,30)
percentile_50=np.percentile(sfr,50)
percentile_70=np.percentile(sfr,70)
percentile_84=np.percentile(sfr,84)
n,bins_hist,patches=plt.hist(sfr,bins=15)
plt.vlines(percentile_16,0,len(sfr),color='red',label='16 percent %.2f'%percentile_16)
plt.vlines(percentile_30,0,len(sfr),color='purple',label='30 percent %.2f'%percentile_30)
plt.vlines(percentile_50,0,len(sfr),color='yellow',label='50 percent %.2f'%percentile_50)
plt.vlines(percentile_70,0,len(sfr),color='black',label='70 percent %.2f'%percentile_70)
plt.vlines(percentile_84,0,len(sfr),color='green',label='84 percent %.2f'%percentile_84)
plt.xlabel('SFR Msol/yr')
plt.title('Halo SFR, nprofs = %s'%str(nprofs))
plt.legend()
plt.ylim(0,max(n)+2)
#plt.savefig('/global/homes/e/emm376/Repositories/illstack/scripts/Results/July_9_'+type+'_'+sim+'_'+snap+'_sfr_histogram_'+mcenter_power+'.pdf', bbox_inches='tight')
plt.close()
...
for i in np.arange(nprofs):
    ...
    if type == 'pres':
        ...
        pass
    """
idx_red=np.where(sfr >= cut_sfr)
idx_blue=np.where(sfr < cut_sfr)
...
#print "idx red", idx_red
#print "idx blue", idx_blue
#print "xvalues", np.shape(x_values)
...
#red is high
if nprofs_red==0:
    print "from showstacks we have 0 profiles with high SFR"
else:
    ...
    #np.savetxt('/global/homes/e/emm376/Repositories/illstack/scripts/Results/Density_'+sim+'_'+snap+'_sfr_high_'+mcenter_power+'.txt', np.c_[mean_xvals_red,y_dens_red,mean_dens_red,errlow_dens_red,errup_dens_red],header=header_dens)
    #np.savetxt('/global/homes/e/emm376/Repositories/illstack/scripts/Results/Pressure_'+sim+'_'+snap+'_sfr_high_'+mcenter_power+'.txt', np.c_[mean_xvals_red,y_pres_red,mean_unnorm_pres_red,errlow_pres_red,errup_pres_red],header=header_pres)

#blue is low
if nprofs_blue == 0:
    print "from showstacks, we have 0 profiles with low SFR"
else:
    ...
    #np.savetxt('/global/homes/e/emm376/Repositories/illstack/scripts/Results/Density_'+sim+'_'+snap+'_sfr_low_'+mcenter_power+'.txt', np.c_[mean_xvals_blue,y_dens_blue,mean_dens_blue,errlow_dens_blue,errup_dens_blue],header=header_dens)
    #np.savetxt('/global/homes/e/emm376/Repositories/illstack/scripts/Results/Pressure_'+sim+'_'+snap+'_sfr_low_'+mcenter_power+'.txt', np.c_[mean_xvals_blue,y_pres_blue,mean_unnorm_pres_blue,errlow_pres_blue,errup_pres_blue],header=header_pres)
...
plt.legend()
plt.title(r'z=%s halos with $ %s < M < %s M_\odot$, SFR cut %s' % (str(z),str(mlow), str(mhigh),str(cut_sfr)))
plt.savefig('/global/homes/e/emm376/Repositories/illstack/scripts/Results/July_5_'+type+'_'+sim+'_'+snap+'_sfr_'+mcenter_power+'.pdf', bbox_inches='tight')
...
if nprofs_red ==0:
    ...
    for r in np.arange(len(GroupFirstSub_red)):
        tree=il.sublink.loadTree(basepath,float(snap),GroupFirstSub_red[r],fields=['SubhaloSFR','SnapNum'],onlyMPB=True)
        sfr=tree['SubhaloSFR']
        plt.plot(tree['SnapNum'],sfr,'-',color='r',alpha=0.5)
if nprofs_blue ==0:
    pass
else:
    for b in np.arange(len(GroupFirstSub_blue)):
        tree=il.sublink.loadTree(basepath,float(snap),GroupFirstSub_blue[b],fields=['SubhaloSFR','SnapNum'],onlyMPB=True)
        sfr=tree['SubhaloSFR']
        plt.plot(tree['SnapNum'],sfr,'-',color='b',alpha=0.4)
...
plt.title('SFR merger,nprofs = %s,cut = %s'%(str(nprofs),str(cut_sfr)))
plt.savefig('/global/homes/e/emm376/Repositories/illstack/scripts/Results/July_11_'+sim+'_'+snap+'_merger_sfr_cut_'+mcenter_power+'.pdf', bbox_inches='tight')
plt.close()
"""
